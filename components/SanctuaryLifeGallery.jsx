...
export default function SanctuaryLifeGallery() {
  ...
  const [galleryItems, setGalleryItems] = useState(DEFAULT_SYSTEM_SETTINGS.galleryImages);
  ...
  // Standard column spans for grid
  const spans = [
    "md:col-span-3 lg:col-span-3 h-[320px] sm:h-[400px]",
    "md:col-span-3 lg:col-span-2 h-[190px] sm:h-[200px]",
    "md:col-span-3 lg:col-span-1 h-[190px] sm:h-[200px]",
    "md:col-span-3 lg:col-span-1 h-[190px] sm:h-[200px]",
    "md:col-span-3 lg:col-span-2 h-[190px] sm:h-[200px]",
    "md:col-span-3 lg:col-span-3 h-[260px] sm:h-[300px]",
  ];
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-12 md:px-16 relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4 sm:gap-6">
          {galleryItems.map((item, idx) => {
            ...
            const isExternal = imgUrl && (imgUrl.startsWith("http") || imgUrl.startsWith("data:"));
            const cardSpan = item.span || spans[idx % spans.length];

            return (
              <div
                key={item.id || idx}
                className={`${cardSpan} relative rounded-[28px] sm:rounded-[36px] overflow-hidden wood-stroke group cursor-pointer bg-slate-900`}
                onClick={() => openLightbox({ ...item, image: imgUrl })}
                style={{
                  opacity: 0,
        ...
        >
          ...
          >
            <div className="relative aspect-[16/9] bg-slate-900">
              {lightbox.image && (lightbox.image.startsWith("http") || lightbox.image.startsWith("data:")) ? (
                // eslint-disable-next-line @next/next/no-img-element
