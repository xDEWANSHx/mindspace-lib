...
import { fetchBranches, fetchMembers } from "@/lib/adminService";
...
export default function DashboardLayout({ children, activeBranch, setActiveBranch, selectedMonth, setSelectedMonth }) {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      setBranches(bList);
      const name = localStorage.getItem("mindspace_user_name") || "Harsh Goyal";
      setUserName(name);

      const savedRole = localStorage.getItem("mindspace_user_role") || "admin";
      setUserRole(savedRole);

      // Calculate live counts for badges
      try {
        ...
        const localLeadsStr = localStorage.getItem("mindspace_leads");
        if (localLeadsStr) {
          ...
          const { data } = await supabase.from('leads').select('*').eq('status', 'new');
          setEnquiriesBadgeCount(data && data.length > 0 ? String(data.length) : null);
        }
      } catch (e) {}
    }
    loadData();
