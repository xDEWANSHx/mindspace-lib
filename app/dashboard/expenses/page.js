...
export default function ExpensesProfitPage() {
  ...
  const handleDeleteAllExpenses = async () => {
    if (!confirm("Are you sure you want to delete ALL expense records? This will clear the entire expense ledger!")) return;
    await deleteAllExpenses(activeBranch);
    await reloadData();
  };
          ...
          <div className="flex flex-wrap items-center gap-3">
            {expenses.length > 0 && (
              <button
                ...
                className="px-3.5 py-2.5 rounded-xl bg-rose-50 border border-rose-200 text-rose-600 hover:bg-rose-100 font-bold text-xs flex items-center gap-1.5 transition-all cursor-pointer"
                title="Delete all expense entries"
              >
