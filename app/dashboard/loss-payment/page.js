...
export default function LossPaymentLedgerPage() {
  ...
  const handlePerformSettle = async (e) => {
    ...
    const targetStudent = selectedMember;
    setSelectedMember(null);
    await reloadData();

    const sendWA = confirm(`Loss payment of ₹${settleAmount} settled for ${targetStudent.full_name}! Would you like to send a WhatsApp receipt to the student now?`);
    if (sendWA) {
      const msg = formatWhatsAppMessage("loss_settlement", {
        student_name: targetStudent.full_name || "Student",
        amount: settleAmount
      });
      openWhatsAppDirectMessage(targetStudent.mobile, msg);
    }
  };
      ...
      <div className="space-y-6 animate-[fadeIn_0.3s_ease-out]">
        ...
        <div className="bg-gradient-to-r from-rose-50 via-white to-amber-50/50 border border-rose-200/80 rounded-3xl p-5 shadow-2xs space-y-2">
          <div className="flex items-center gap-2 text-rose-900 font-extrabold text-xs">
            ...
            <span>Defaulter Loss Tracking & Debt Recovery Logic:</span>
          </div>
          <p className="text-xs text-slate-700 leading-relaxed">
            This section tracks all-time records of students who departed with outstanding fee dues.
            When a student is marked <strong>Left with Dues</strong>, their assigned seat and locker are immediately vacated for new enrollments, while their unpaid balance is archived here.
            If the student returns to clear their balance later, click <strong>Settle Loss Fee</strong> to record the recovery payment, which will automatically credit back into <strong>Total Revenue</strong>.
          </p>
        </div>
