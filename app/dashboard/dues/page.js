          ...
          {duesMembers.length === 0 ? (
            ...
            duesMembers.map(m => {
              ...
              const waMsg = formatWhatsAppMessage(tplKey, {
                ...
                seat_no: m.seat_no || "N/A",
                shift: m.shift || "Full Day",
                expiry_date: m.subscription_end_date ? m.subscription_end_date.substring(0, 10) : "N/A",
                dues_amount: duesVal
              });
              ...
              return (
                <div key={m.id} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-sm flex flex-col justify-between space-y-4 hover:shadow-md transition-all">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div className="bg-amber-50 border border-amber-100 rounded-xl p-2.5 flex items-center gap-2">
                      ...
                      <div className="truncate">
                        <span className="text-[9px] uppercase font-bold text-amber-600 block">
                          {m.outstanding_dues > 0 && (m.due_date || m.dues_due_date) ? "Promised Date" : "Expiry Date"}
                        </span>
                  ...
                  <div className="pt-2 border-t border-slate-100">
                    <button
                      type="button"
                      onClick={() => openWhatsAppDirectMessage(m.mobile, waMsg)}
                      className="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[#25D366] hover:bg-[#1db954] text-white font-bold text-xs shadow-sm transition-all cursor-pointer"
                    >
