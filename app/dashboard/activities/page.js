...
import { fetchActivityLogs, clearAllActivityLogs, subscribeToCloudChanges } from "@/lib/adminService";
...
export default function AuditTrailLogPage() {
  ...
  const handleClearLogs = async () => {
    if (confirm("Are you sure you want to clear all activity audit logs? This cannot be undone.")) {
  ...
  const filteredLogs = logs.filter(l => {
    ...
    if (activeTab === "STUDENTS") {
      if (!action.includes("student") && !action.includes("member") && !action.includes("admission") && !action.includes("readmission") && !action.includes("subscription") && !action.includes("update") && !details.includes("student") && !details.includes("admitted")) return false;
    } else if (activeTab === "PAYMENTS") {
      if (!action.includes("payment") && !action.includes("loss") && !action.includes("invoice") && !action.includes("due") && !action.includes("settled")) return false;
    } else if (activeTab === "SEATING") {
      if (!action.includes("seat") && !action.includes("locker") && !action.includes("shift") && !details.includes("seat") && !details.includes("locker")) return false;
    } else if (activeTab === "EXPENSES") {
      if (!action.includes("expense")) return false;
    } else if (activeTab === "SETTINGS") {
      if (!action.includes("setting") && !action.includes("password") && !action.includes("enquiry") && !action.includes("lead")) return false;
    }
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2.5 rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-extrabold text-xs flex items-center gap-2 border border-rose-200 shadow-2xs transition-all cursor-pointer"
            >
