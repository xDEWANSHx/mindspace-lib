...
export function sanitizeMemberForSupabase(m) {
  ...
  for (const col of allowedColumns) {
    if (m[col] !== undefined) {
      let val = m[col];
      if (dateColumns.includes(col) && (val === "" || (typeof val === 'string' && val.trim() === ""))) {
        val = null;
      }
      sanitized[col] = val;
    }
...
export async function fetchMembers(branch = 'main_branch') {
  ...
  // Auto-cap inflated dues exceeding member plan_amount
  let hasDuesFixed = false;
  members = members.map(m => {
    const pAmt = parseFloat(m.plan_amount || 1100);
    const currDues = parseFloat(m.outstanding_dues || 0);
    if (currDues > pAmt) {
      hasDuesFixed = true;
      return { ...m, outstanding_dues: pAmt };
    }
    return m;
  });
...
export async function createMember(memberData, actor = 'Admin') {
  ...
  const subEndDate = memberData.subscription_end_date || formatDate(new Date(Date.now() + 30 * 86400000));
...
export async function fetchPayments(branch = 'main_branch') {
  ...
  return payments
    .filter(p => p.branch === branch || !p.branch)
    .sort((a, b) => {
  ...
  if (member_id) {
    ...
    if (member) {
      ...
      if (end_date) {
        ...
        let baseDateStr = start_date || paymentDateStr;
        if (!start_date && member.subscription_end_date && member.subscription_end_date >= paymentDateStr) {
          baseDateStr = addDaysToDate(member.subscription_end_date, 1);
        }

        if (parseInt(extend_days) === 30 || extend_days === '30') {
          updates.subscription_end_date = addOneMonth(baseDateStr);
        } else {
          updates.subscription_end_date = addDaysToDate(baseDateStr, extend_days);
        }
      }

      if (start_date && !member.joining_date) {
        updates.joining_date = start_date;
      }
