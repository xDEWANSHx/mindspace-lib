...
export function getMemberSubscriptionDates(member, payments = []) {
  ...
  if ((!paymentList || paymentList.length === 0) && typeof window !== 'undefined') {
    paymentList = getLocal('mindspace_payments', []);
  }
  if (!Array.isArray(paymentList)) paymentList = [];

  const memPayments = paymentList.filter(p =>
  ...
  if (memPayments.length === 0) {
    const subStart = (member.joining_date && String(member.joining_date).trim() !== "")
      ? String(member.joining_date).substring(0, 10)
      : "--";
    return { initialAdmissionDate, subStart, subExpiry: "--" };
  }
  ...
  // Only fallback to member.joining_date if subStart is completely missing
  if (!subStart && member.joining_date && String(member.joining_date).trim() !== "") {
    subStart = String(member.joining_date).substring(0, 10);
  }
  ...
  let subExpiry = rawSubEnd;
  if (!subExpiry && latestP.notes) {
    const matchExp = latestP.notes.match(/Expiry:\s*(\d{4}-\d{2}-\d{2})/i);
    if (matchExp && matchExp[1]) subExpiry = matchExp[1];
  }
  if (!subExpiry && subStart) {
    subExpiry = addOneMonth(subStart);
  }
...
// -------------------------------------------------------------
export function calculateMemberStatus(member, payments = null) {
  ...
  if (member.is_active) {
    ...
    let paymentList = payments;
    if (!paymentList && typeof window !== 'undefined') {
      paymentList = getLocal('mindspace_payments', []);
    }
    if (!Array.isArray(paymentList)) paymentList = [];

    const memPayments = paymentList.filter(p =>
      (member.id && p.member_id === member.id) ||
      (member.permanent_id && p.member_id === member.permanent_id) ||
      (member.student_no && p.member_id === member.student_no) ||
      (p.member_name && member.full_name && p.member_name.trim().toLowerCase() === member.full_name.trim().toLowerCase())
    );

    const hasAnyPayment = memPayments.length > 0;
    const isHalfDay = member.shift === 'Morning' || member.shift === 'Evening';
    const defaultFee = isHalfDay ? 600 : 1100;
    const lockerAdd = member.has_locker ? 50 : 0;
    const planFee = (member.plan_amount || defaultFee) + lockerAdd;

    // If payments list exists and student has 0 payment records, effective dues is their plan fee
    const effectiveDues = (member.outstanding_dues > 0)
      ? member.outstanding_dues
      : (paymentList.length > 0 && !hasAnyPayment ? planFee : 0);

    // 1. Promised Due Date Overdue Check (If student has outstanding dues and promised payment date has passed)
    ...
    // 2. Subscription Expiry Overdue Check (Only valid if student actually has paid records or no payment list available)
    const hasValidSub = (paymentList.length === 0 || hasAnyPayment) && member.subscription_end_date && !String(member.subscription_end_date).startsWith("1970");
    if (hasValidSub) {
...
export async function createMember(memberData, actor = 'Admin') {
  ...
  const permId = memberData.permanent_id || `#MS26B${Date.now().toString().slice(-4)}`;

  const subEndDate = memberData.subscription_end_date && !String(memberData.subscription_end_date).startsWith('1970')
    ? memberData.subscription_end_date
    : (memberData.joining_date ? addOneMonth(memberData.joining_date) : addOneMonth(new Date()));

  const newMember = {
    id: newId,
    student_no: permId,
    joining_date: memberData.joining_date || formatDate(new Date()),
...
export async function updateMember(id, updates, actor = 'Admin', beforeMember = null) {
  ...
  if (updatedMember) {
    // Backup sync to LocalStorage
    const currentMembersLocal = getLocal('mindspace_members', []);
    const updatedMembersLocal = currentMembersLocal.map(m => m.id === id ? { ...m, ...updates, ...updatedMember } : m);
    if (!updatedMembersLocal.some(m => m.id === id)) {
      updatedMembersLocal.unshift(updatedMember);
    }
    setLocal('mindspace_members', updatedMembersLocal);
...
export async function deletePayment(paymentId, actor = 'Admin') {
  ...
  // Also check local storage payments if not found via supabase
  const currentPaymentsLocal = getLocal('mindspace_payments', []);
  if (!beforeState) {
    beforeState = currentPaymentsLocal.find(p => p.id === paymentId);
  }
  ...
  // Always update LocalStorage payments
  const updatedPaymentsLocal = currentPaymentsLocal.filter(p => p.id !== paymentId);
  setLocal('mindspace_payments', updatedPaymentsLocal);

  if (beforeState && (beforeState.member_id || beforeState.member_name)) {
    const branch = beforeState.branch || 'main_branch';
    const members = await fetchMembers(branch);
    const member = members.find(m =>
      (beforeState.member_id && (
        String(m.id).toLowerCase() === String(beforeState.member_id).toLowerCase() ||
        String(m.permanent_id || '').toLowerCase() === String(beforeState.member_id).toLowerCase() ||
        String(m.student_no || '').toLowerCase() === String(beforeState.member_id).toLowerCase()
      )) ||
      (beforeState.member_name && m.full_name && m.full_name.trim().toLowerCase() === beforeState.member_name.trim().toLowerCase())
    );

    if (member && member.is_active !== false) {
      // Fetch all remaining payments for this member across database and local
      const allBranchPayments = await fetchPayments(branch);
      const remPayments = allBranchPayments.filter(p =>
        p.id !== paymentId && (
          (member.id && (p.member_id === member.id || p.member_id === member.permanent_id || p.member_id === member.student_no)) ||
          (member.permanent_id && (p.member_id === member.permanent_id || p.member_id === member.id)) ||
          (member.full_name && p.member_name && p.member_name.trim().toLowerCase() === member.full_name.trim().toLowerCase())
        )
      );
      ...
      const isHalfDay = member.shift === 'Morning' || member.shift === 'Evening';
      const defaultBasePrice = isHalfDay ? 600 : 1100;
      const lockerAdd = member.has_locker ? 50 : 0;
      const planAmt = (member.plan_amount !== undefined && member.plan_amount !== null && !isNaN(parseFloat(member.plan_amount)))
        ? parseFloat(member.plan_amount)
        : (defaultBasePrice + lockerAdd);

      let newSubEndDate = null;
      let restoredDues = planAmt;
      let newPaymentStatus = 'UNPAID';
      let newDueDate = member.due_date || null;

      if (remPayments.length === 0) {
        newSubEndDate = null; // Unset subscription expiry date when all payments are deleted
        restoredDues = planAmt; // Restore dues to base plan amount
        newPaymentStatus = 'UNPAID';
        newDueDate = member.due_date || null;
      } else {
        const lastPayment = remPayments[remPayments.length - 1];
        
        // 1. Recalculate subscription end date from last remaining payment
        let extractedExpiry = null;
        if (lastPayment.notes) {
          const matchExp = lastPayment.notes.match(/Expiry:\s*(\d{4}-\d{2}-\d{2})/i);
          if (matchExp && matchExp[1]) extractedExpiry = matchExp[1];
        }
        if (extractedExpiry) {
          newSubEndDate = extractedExpiry;
        } else {
        ...
        // 2. Recalculate dues from last remaining payment
        const isLastPayLater = lastPayment.payment_mode === 'Deferred' || (lastPayment.notes && /pay\s*later|deferred/i.test(lastPayment.notes));
        if (isLastPayLater) {
          restoredDues = planAmt;
          newPaymentStatus = 'PAY_LATER';
          newDueDate = member.due_date || null;
        } else if (lastPayment.notes) {
          const matchDues = lastPayment.notes.match(/₹?\s*(\d+(?:\.\d+)?)\s*(?:Total\s+Overdue\s+Dues|Overdue\s+Dues|Total\s+Dues|Remaining\s+Dues|Dues\s+Remaining|Dues\s+Pending|Dues\s+Left|Pending\s+Dues)/i);
          if (matchDues && matchDues[1] !== undefined) {
            restoredDues = parseFloat(matchDues[1]);
            newPaymentStatus = restoredDues > 0 ? 'PARTIAL' : 'PAID';
            newDueDate = restoredDues > 0 ? (member.due_date || null) : null;
          } else {
            restoredDues = 0;
      ...
      // Always pass member.id (the Postgres primary key UUID)
      await updateMember(member.id, updates, actor, member);
    }
  }
...
export async function recordPayment({ member_id, member_name, amount, branch = 'main_branch', payment_mode = 'Cash', cash_amount = 0, online_amount = 0, notes = '', is_renewal = false, extend_days = 30, reduce_dues = 0, new_outstanding_dues = null, dues_due_date = null, start_date = null, end_date = null, has_locker = null, paid_at = null, actor = 'Admin' }) {
  ...
  if (member_id) {
    ...
    if (member) {
      ...
      if (is_renewal || !member.subscription_end_date || String(member.subscription_end_date).startsWith("1970")) {
