...
export async function deletePayment(paymentId, actor = 'Admin') {
  ...
  if (beforeState && beforeState.member_id) {
    ...
    if (member && member.is_active !== false) {
      ...
      const planAmt = (member.plan_amount !== undefined && member.plan_amount !== null && !isNaN(parseFloat(member.plan_amount))) ? parseFloat(member.plan_amount) : (defaultBasePrice + lockerAdd);
