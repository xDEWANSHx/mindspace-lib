...
export function getMemberSubscriptionDates(member, payments = []) {
  ...
  if (memPayments.length === 0) {
    const subStart = (member.joining_date && String(member.joining_date).trim() !== "")
      ...
      : (rawSubEnd !== "--" ? subtractOneMonth(rawSubEnd) : "--");
  ...
  // Only fallback to member.joining_date or subtractOneMonth if subStart is completely missing
  if (!subStart && member.joining_date && String(member.joining_date).trim() !== "") {
    subStart = String(member.joining_date).substring(0, 10);
  } else if (!subStart && rawSubEnd !== "--") {
