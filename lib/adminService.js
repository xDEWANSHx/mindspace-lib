...
export function getMemberSubscriptionDates(member, payments = []) {
  ...
  if (memPayments.length === 0) {
    const subStart = rawSubEnd !== "--" ? subtractOneMonth(rawSubEnd) : (initialAdmissionDate !== "--" ? initialAdmissionDate : "--");
    return { initialAdmissionDate, subStart: subStart || initialAdmissionDate || "--", subExpiry: rawSubEnd };
  }
  ...
  // Only fallback to subtractOneMonth if subStart is completely missing
  if (!subStart && rawSubEnd !== "--") {
    subStart = subtractOneMonth(rawSubEnd);
  }

  const subExpiry = rawSubEnd !== "--" ? rawSubEnd : (subStart ? addOneMonth(subStart) : "--");
  return { initialAdmissionDate, subStart: subStart || initialAdmissionDate || "--", subExpiry: subExpiry || "--" };
}
...
export async function updateMember(id, updates, actor = 'Admin', beforeMember = null) {
  ...
  // Auto-adjust dues if plan_amount changed AND updates.outstanding_dues was NOT explicitly specified
  if (beforeState && updates.plan_amount !== undefined && updates.outstanding_dues === undefined) {
  ...
  if (updatedMember) {
    ...
    const updatedLocal = currentLocal.map(m => {
      if (m.id === id || (m.permanent_id && m.permanent_id === updatedMember.permanent_id)) {
        found = true;
    ...
    if (!found) {
      updatedLocal.unshift(updatedMember);
    }
    setLocal('mindspace_members', updatedLocal);
...
export async function updatePaymentRecord(paymentId, updateData, actor = 'Admin') {
  ...
  if (currentPayment && currentPayment.member_id && updateData.paid_at && !updateData.skipMemberSync) {
    ...
    if (member) {
      const pDate = String(updateData.paid_at).substring(0, 10);
      const newSubEnd = updateData.end_date || updateData.subscription_end_date || addOneMonth(pDate);
...
export async function recordPayment({ member_id, member_name, amount, branch = 'main_branch', payment_mode = 'Cash', cash_amount = 0, online_amount = 0, notes = '', is_renewal = false, extend_days = 30, reduce_dues = 0, new_outstanding_dues = null, dues_due_date = null, start_date = null, end_date = null, has_locker = null, paid_at = null, actor = 'Admin' }) {
  ...
  const calculatedEnd = end_date || (is_renewal ? addOneMonth(subStartStr) : null);
  const formattedNotes = notes
    ? (notes.includes("Start Date:") ? notes : (is_renewal ? `${notes} — Start Date: ${subStartStr}, Expiry: ${calculatedEnd}` : notes))
    : (is_renewal ? `Subscription Payment — Start Date: ${subStartStr}, Expiry: ${calculatedEnd}` : `Dues Recovery Payment — Amount: ₹${parsedAmt}`);
  ...
  if (member_id) {
    ...
    if (member) {
      ...
      } else if (end_date && !String(end_date).startsWith("1970") && !member.subscription_end_date) {
        updates.subscription_end_date = end_date;
      }

      if (!member.joining_date && is_renewal) {
